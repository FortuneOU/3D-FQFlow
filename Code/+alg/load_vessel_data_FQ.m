function [outStruct, Grid] = load_vessel_data_FQ(filename, vtuProperties)
...
try
    ...
    warning('Sparse creation failed: %s\nFalling back to mapping structure.', ME.message);
    
    % 保存映射信息
    Grid.vtu_indices_keys = ind;
    Grid.vtu_indices_vals = 1:length(ind);
    
    % 定义一个函数句柄来模拟索引行为
    Grid.vtu_indices = @(idx) vessel_index_lookup(idx, Grid.vtu_indices_keys, Grid.vtu_indices_vals);
    Grid.vtu_indices_isSparse = false;
end
...
function val = vessel_index_lookup(idx, keys, vals)
    [tf, loc] = ismember(idx, keys);
    val = zeros(size(idx));
    val(tf) = vals(loc(tf));
end
